...
function(combX, combZ, combL, statistic, grid.size, nGridSize, timeIGSBreakDown, takeN, verbose, timing) {
	...
	for(g in 1:length(grid.size)) {
		...
		combZCumSum = as.numeric(.Call("ScanIGSGridCumSumC", as.numeric(combZ), as.numeric(grid.cur)))
		combXCumSum = as.numeric(.Call("ScanIGSGridCumSumC", as.numeric(combX), as.numeric(grid.cur)))
